...
def test_bank_all_valid_and_unique():
    ...
    assert len(codes) == len(set(codes)) == 300 + 50 * len(BATCH2_DIMS)
    for q in items:
        validate_question(q)
...
BATCH2_DIMS = ["D1", "D2"]  # 批次 2 按任务逐维扩展：Task1 D1+D2 → Task2 +D3+D4 → Task3 +D5+D6
...
def test_bank_batch2_quota():
    ...
    id 续号 B56..B83 / H25..H36 / A22..A31，难度金字塔精确。"""
    items = load_seed_files(SEEDS)
    assert len(items) == 300 + 50 * len(BATCH2_DIMS)
    for dim in BATCH2_DIMS:
        ...
        new_basic = [q for q in basic if _id_num(q["id"]) >= 56]
        new_adv_obj = [q for q in adv_obj if _id_num(q["id"]) >= 25]
        ...
        new_subj = [q for q in qs if q["type"] in ("open", "practical") and _id_num(q["id"]) >= 22]

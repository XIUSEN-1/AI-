...
## 15. M2 增补（2026-09-12 第二轮 grilling 定案）

1. **考级模式不做**：现有 L1-L5 自适应映射即等级体系。
2. **题库分三批扩容至 1000**：300 → 600 → 1000；每批流程 = 批量生成 → 结构校验 + 查重测试（题干 n-gram 相似度）→ 事实抽查过审 → 扩下一批；主:客 ≈ 2:8；难度金字塔分布（低难多高难少）；每批结束可停在当前规模交付。
3. **分级建议库 30 格**（`seeds/advice_matrix.json`，版本化）：每维度 × L1~L5 一格，含针对性文案、真实学习资源（课程/文档链接）、练习任务、晋级标准；报告生成按等级命中格子，LLM 基于格子内容个性化组装（无 LLM 时直接渲染格子文案）；不收集学历等背景信息（隐私最小化）。
4. **错题本 + 练习卷**：`GET /api/me/wrongbook`（错题归集，按维度/考点标签）；`POST /api/practice/sessions`（薄弱考点练习卷：从错题维度/标签抽题，即时判分，不更新 θ、不计等级）；前端工作台入口 + 练习页。
5. **排期**：M2b（对话式+实操+编排）→ M2e（千题批次1 + 分级建议库 + 错题本/练习卷）→ M2c（教师端/后台）→ M2d（校准/趋势/导出/部署）；千题批次 2/3 插在 M2c/d 期间。

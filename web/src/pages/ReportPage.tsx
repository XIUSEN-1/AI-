...
interface MineItem {
  ...
  total_level: number;
  total_level_name: string;
  avg_percent: number | null; // 旧报告维度明细缺失时可能为空
}
...
// 六维 percent 均值（综合百分制），与后端 mine.avg_percent 同口径
function avgPercent(dimensions: DimensionDetail[]): number {
  return Math.round(dimensions.reduce((s, d) => s + d.percent, 0) / dimensions.length);
}
...
export default function ReportPage() {
  ...
  // 历次报告（成长趋势）：辅助信息，拉取失败不打断报告主体，console 上报
  useEffect(() => {
    api<MineItem[]>("/api/reports/mine")
      .then(setMine)
      .catch((e: unknown) => console.error("成长趋势加载失败：", e));
  }, []);

  // 趋势点：mine 历史 + 当前报告兜底（后端 mine 倒序返回，这里转时间正序）
  const trendData = useMemo(() => {
    const points = mine
      .filter((m): m is MineItem & { avg_percent: number } => m.avg_percent != null)
      .map((m) => ({ id: m.id, created_at: m.created_at, avg_percent: m.avg_percent }));
    if (report && !points.some((p) => p.id === report.id)) {
      points.push({ id: report.id, created_at: report.created_at, avg_percent: avgPercent(report.dimensions) });
    }
    points.sort((a, b) => a.created_at.localeCompare(b.created_at));
    return points.map((p) => ({ date: p.created_at.slice(5, 10), avg_percent: p.avg_percent }));
  }, [mine, report]);
  ...
  async function handleExport() {
    ...
    try {
      ...
      link.download = `AI能力罗盘报告-${new Date().toISOString().slice(0, 10)}.png`;
      link.href = dataUrl;
      link.click();
    } catch (e: unknown) {
      setExportError(e instanceof Error ? `导出失败：${e.message}` : "导出失败，请重试");
    } finally {
      setExporting(false);
    }
  }
